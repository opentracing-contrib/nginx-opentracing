...
docker-build-binaries:
	DOCKER_BUILDKIT=1 docker buildx build --build-arg NGINX_VERSION=1.26.1 --platform linux/amd64 -f build/Dockerfile -t nginx-opentracing-binaries --target=export --output "type=local,dest=out" --progress=plain --no-cache --pull .
