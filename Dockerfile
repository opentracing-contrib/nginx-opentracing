...
FROM --platform=$BUILDPLATFORM debian:13.6 AS build-base-debian
...
FROM nginx:1.31.4 AS build-nginx-debian
...
FROM nginx:1.31.4-alpine AS build-nginx-alpine
RUN apk add --no-cache \
    build-base \
    pcre2-dev \
    zlib-dev
...
FROM nginx:1.31.4-alpine AS nginx-alpine
RUN apk add --no-cache libstdc++


### Base image for debian
FROM nginx:1.31.4 AS nginx-debian
