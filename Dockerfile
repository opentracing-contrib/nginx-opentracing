...
FROM nginx:1.25.2 as build-nginx-debian
...
FROM nginx:1.25.2-alpine AS build-nginx-alpine
RUN apk add --no-cache \
    build-base \
    pcre2-dev \
    zlib-dev
...
FROM nginx:1.25.2-alpine as nginx-alpine
RUN apk add --no-cache libstdc++


### Base image for debian
FROM nginx:1.25.2 as nginx-debian
