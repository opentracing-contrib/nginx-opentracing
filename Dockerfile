...
FROM --platform=$BUILDPLATFORM debian:13.5 AS build-base-debian
...
FROM --platform=$BUILDPLATFORM alpine:3.24 AS build-base-alpine
...
FROM nginx:1.31.2 AS build-nginx-debian
...
FROM nginx:1.31.2-alpine AS build-nginx-alpine
RUN apk add --no-cache \
    build-base \
    pcre2-dev \
    zlib-dev
...
FROM nginx:1.31.2-alpine AS nginx-alpine
RUN apk add --no-cache libstdc++


### Base image for debian
FROM nginx:1.31.2 AS nginx-debian
