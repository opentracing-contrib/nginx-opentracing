...
FROM nginx:1.30.0 AS build-nginx-debian
...
FROM nginx:1.30.0-alpine AS build-nginx-alpine
RUN apk add --no-cache \
    build-base \
    pcre2-dev \
    zlib-dev
...
FROM nginx:1.30.0-alpine AS nginx-alpine
RUN apk add --no-cache libstdc++


### Base image for debian
FROM nginx:1.30.0 AS nginx-debian
