...
FROM nginx:1.25.0 as build-nginx-debian
...
FROM nginx:1.25.0-alpine AS build-nginx-alpine
RUN apk add --no-cache \
    build-base \
    pcre2-dev \
    zlib-dev
...
FROM nginx:1.25.0-alpine as nginx-alpine
RUN apk add --no-cache libstdc++


### Base image for debian
FROM nginx:1.25.0 as nginx-debian
