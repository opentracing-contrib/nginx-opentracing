...
FROM --platform=$BUILDPLATFORM debian:trixie-20260406 AS build-base-debian
...
FROM --platform=$BUILDPLATFORM alpine:3.23 AS build-base-alpine
...
FROM nginx:1.29.8 AS build-nginx-debian
...
FROM nginx:1.29.8-alpine AS build-nginx-alpine
RUN apk add --no-cache \
    build-base \
    pcre2-dev \
    zlib-dev
...
FROM nginx:1.29.8-alpine AS nginx-alpine
RUN apk add --no-cache libstdc++


### Base image for debian
FROM nginx:1.29.8 AS nginx-debian
